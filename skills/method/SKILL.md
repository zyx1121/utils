---
description: "INVOKE FIRST at the start of any non-trivial coding or decision task to classify the task and load the matching procedure. Routes to: RCA for debug, diagnosis pre-edit anchor, CoVe verification, Working Backwards for vague features, Steelman for option picks, ADR for architecture, Tidy-First for refactor, Profile-first for perf, Strangler for migration, Source-first for research, Threat-model for security, Cunningham for stuck, Consumer-first for API design. STRONG triggers (user phrases): '2+ failed fixes', 'why isn't this working', '為什麼還不行', '怎麼又錯了', 'should work / probably correct', '應該可以', traceback / test red / 線上異常, 'how does X work' / '怎麼運作', 'should we use A or B' / '要選 A 還是 B', '怎麼這麼慢' / latency complaint, breaking change / deprecate / migration, auth / permission / RLS / 權限 edits, new feature with vague spec, refactor / cleanup / 整理, API / interface / function signature design, stuck > 30 min / 卡住. Skip ONLY for genuinely trivial single-action tasks (rename, format, single file read, simple lookup)."
---
