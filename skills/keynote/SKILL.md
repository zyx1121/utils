...
utils keynote save ~/Desktop/<deck>.key
```

For pptx-only templates: `open` accepts pptx; one `save <path>.key` converts.
...
- **`set-title` / `set-body` only touch the layout's default placeholders.** Decks with extra text boxes per slide need finer access — not yet in `utils keynote`; drop to raw `osascript` for that.
